...
void main() => runApp(const MyShowCase());
...
class MyShowCase extends StatefulWidget {
  const MyShowCase({super.key});

  @override
  State<MyShowCase> createState() => _MyShowCaseState();
}

class _MyShowCaseState extends State<MyShowCase> {
  ...
  void initState() {
    super.initState();
    // _checkAndRunShowcase();
    ShowcaseView.register(
      scope: 'home',
      onFinish: () async {
        // Save flag so it doesn’t run again
      },
    );

    // Start showcase after first frame
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ShowcaseView.getNamed('home').startShowCase([_one, _two]);
    });
  }

  Future<void> _checkAndRunShowcase() async {
    final prefs = await SharedPreferences.getInstance();
    final alreadyShown = prefs.getBool('showcase_shown') ?? false;
    debugPrint('alreadyShown? $alreadyShown');
    if (!alreadyShown) {
      // Register the scope
      ShowcaseView.register(
        scope: 'home',
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(primaryColor: const Color(0xffEE5366)),
                    ...
                    container: Container(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          const SizedBox(height: 10),
                          const Text("Your sender's profile"),
                        ],
                      ),
                    ),
              ...
              floatingActionWidget: FloatingActionWidget(
                ...
                child: Padding(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed:
                        ShowcaseView.getNamed(scopeName).dismiss,
                    child: const Text(
                      'Close Showcase',
                      style: TextStyle(
                        color: Colors.white,
